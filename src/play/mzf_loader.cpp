...
/*
   MZ700 header-only UL from current master. It explicitly drives E003=2
   (physical WRITE high) while READ is held at the start gate.
*/
static const uint8_t loader_body_mz700_working_P[] PROGMEM =
{
    0x11, 0x02, 0xE0, 0xF3,
    0x7B, 0x13, 0x12, 0x1B, /* E003=2 -> physical WRITE high */
...
{
    /*
       Keep the proven classic UL receiver/tail byte-for-byte. The only
       addition is a 4-byte MZ700 compatibility-mode switch before it.
    */
...
static uint16_t mzf_loader_build_working_generic(uint8_t *destination)
{
    ...
    /* Force MZ700 compatibility before the proven $E002/$E003 receiver. */
    for (uint8_t i = 0U; i < sizeof(generic_ul_mz700_mode_P); ++i)
    {
        destination[offset++] =
            (uint8_t)pgm_read_byte(generic_ul_mz700_mode_P + i);
    }
...
static void set_ul700_data_error(char edge, uint8_t pair)
{
    ...
    context.error_text[4] = ' ';
    context.error_text[5] = edge;
    context.error_text[6] = (char)('0' + (pair & 0x03U));
    context.error_text[7] = ' ';
...
static void set_ul700_end_error(void)
{
    ul700_diag_prefix(context.error_text);
    context.error_text[4] = ' ';
    context.error_text[5] = 'E';
    context.error_text[6] = 'N';
    context.error_text[7] = 'D';
    context.error_text[8] = ' ';
                           ...
                           uint32_t data_offset)
{
    ...
    {
        /* FASTIPL metadata has BLCOUNT at $18, not the original MZF type.
           Restrict synthetic IC to machine code so the logical type cannot
           be silently lost during a later recording/normalization pass. */
...
bool mzf_loader_patch_loader_header(uint8_t *header)
{
        ...
        is_ic_variant(context.variant))
    {
        ...
        */
        ...
        {
            /* Authentic FASTIPL layout: $18=BLCOUNT, $19=READPOINT. */
            header[MZF_LOADER_METADATA_OFFSET] = 0x01U;
            header[MZF_LOADER_METADATA_OFFSET + 1U] = ic_speed_byte();
        }
        else
        {
            /* Preserve the existing MZ800 header-only UL metadata. */
...
uint16_t mzf_loader_build_loader(uint8_t *destination, uint16_t capacity)
{
    ...
    if (is_mz700_ul_variant(context.variant))
    {
        ...
        for (uint8_t i = 0U;
             i < (uint8_t)(sizeof(loader_body_mz700_working_P) - 1U);
             ++i)
        {
            destination[offset++] =
                pgm_read_byte(loader_body_mz700_working_P + i);
        }
...
bool mzf_loader_begin(void)
{
    ...
    if (is_mz700_ul_variant(context.variant))
    {
        /* Keep current-master UL700: gate closed before SD access and use the
           monotonic WRITE/READ rendezvous on the real first payload byte. */
        ...
        context.first_byte_pending = false;
        context.start_time = 0UL;
        context.start_state = MZF_LOADER_START_TRANSFER;
        context.started = true;
        return true;
    }

    /* UL and UL800 use the hardware-proven 6ffd55d path. */
    if (!sdcard_file_seek(context.data_offset))
    {
        set_error_P(PSTR("UL SEEK"));
        return false;
    ...
    context.start_time = millis();
    context.start_state = MZF_LOADER_START_DELAY;
    return true;
}
    ...
    switch (context.start_state)
    {
        case MZF_LOADER_START_DELAY:
        {
            const uint32_t delay_ms =
                (context.variant == MZF_LOADER_VARIANT_MZ800_HEADER) ?
                    MZF_LOADER_HEADER_START_DELAY_MS :
                    MZF_LOADER_START_DELAY_MS;

            if ((uint32_t)(millis() - context.start_time) < delay_ms)
            {
                return true;
            }
        ...
        case MZF_LOADER_START_READY_DELAY:
            if ((uint32_t)(micros() - context.start_time) <
                MZF_LOADER_HEADER_READY_DELAY_US)
            {
                return true;
            }

            /* Legacy send read byte 0 before beginning its first WRITE wait. */
            received = sdcard_file_read(&context.first_byte, 1U);
            if (received != 1)
            {
                set_error_P((received < 0) ? PSTR("UL READ") :
                                              PSTR("UL SHORT"));
                return false;
            }
            context.first_byte_pending = true;
            context.start_time = micros();
            context.start_state = MZF_LOADER_START_INITIAL_WAIT;
            return true;
        ...
        case MZF_LOADER_START_INITIAL_WAIT:
            ...
            if ((uint32_t)(micros() - context.start_time) >
                MZF_LOADER_HEADER_INITIAL_TIMEOUT_US)
            {
                set_error_P(PSTR("UL WAIT"));
                return false;
            }
...
bool mzf_loader_pump(uint8_t max_bytes)
{
    ...
    if (!mzf_loader_service_startup())
    {
        return false;
    }
    if (context.start_state != MZF_LOADER_START_TRANSFER)
    {
        return true;
    }
    ...
    {
        if (!send_byte_legacy(context.first_byte))
        {
            return false;
        }
        context.first_byte_pending = false;
    ...
    {
        request = (uint16_t)((uint32_t)context.data_length -
                             context.transferred);
        ...
        if (mz700_ul)
        {
            for (uint16_t i = 0U; i < request; ++i)
            {
                ...
                else if (!send_byte_mz700(work[i]))
                {
                    return false;
                }
                context.transferred++;
    ...
    if (context.transferred >= context.data_length)
    {
        ...
        if (mz700_ul)
        {
            if (!wait_write_level_mz700_end(0U)) return false;
        }
